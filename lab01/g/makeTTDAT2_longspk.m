function [T,WV,n] = makeTTDAT2_longspk(chan,file_dat)
...
LAN.srate = 20000;
cfg = struct('sr', LAN.srate, 'w_pre', 20, 'w_post', 20,...
...
n = length(T);
    ...
    function [T,WV] = myoriginalLoadTT(x, TS, chan)
        ...
        WV = nan(length(T),4,cfg.w_pre*2);
        for t = 1:length(T)
            WV(t,:,:) = permute(x(chan,T(t)-(cfg.w_pre-1):T(t)+cfg.w_pre),[3 1 2]);
        end
    end
end