...
if filtorder*3 > epochframes,   % Matlab filtfilt() restriction
    ...
    %fprintf('eegfilt(): filter order is %d and epochframes is %d (%.2f). ',filtorder,epochframes,epochframes/filtorder);
    %error('epochframes must be at least 3 times the filtorder.');
    filtorder = 3*epochframes;
end
...
filtwts = firls(filtorder,f,double(m)); % get FIR filter coefficients
