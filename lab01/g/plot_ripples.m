...
disp('loading ripples data...')
% load([base_dir 'ripples_data_raw_SD_5.mat'])
% load('ripples_data_raw_SD_5__2.mat')
load([base_dir 'ripples_data_raw_SD_3.mat'])
...
disp('loading LFP of the selected session...')
...
if isempty(dir('LFPf_100_250Hz.mat'))
    disp('filtering LFP between 100-250 Hz...')
    ripples = nan(size(data));
    for ch = 1:size(data,1)
        disp(['channel ' num2str(ch) '/' num2str(size(data,1)) '...'])
        ripple = eegfilt(data(ch,:),srate,100,250);
        ripples(ch,:) = ripple;
    end
    disp('saving filtered LFP...')
    save('LFPf_100_250Hz.mat','ripples','srate','channels','tetrodes','-v7.3')
else
    disp('loading LFP filtered between 100-250 Hz...')
    load('LFPf_100_250Hz.mat')
end
% theta = eegfilt(data,srate,6,10);
disp('data ready for plotting.')
beep
pause(.5)
beep
cd([base_dir exp_dir(n_rat).name '\' sessions(n_ses).name])
%%
% SELECCIONA EL CANAL (n_ch)
% figure(2)
n_ch = 9; % elijo el canal a plotear
...
ch = find(RT.est(1,:) == n_ch); % crea un vector logico con los ripples detectados que pertenecen a este canal
% disp([num2str(size(ch,2)) ' ripples in this channel'])
time = ((1:size(data(n_ch,:),2)))/srate; % vector de tiempo para el ploteo

% ELIGE EL RIPPLE QUE QUIERES PLOTEAR (n_ripp)
n_ripp = 126; % aqui seleccionas el ripple de esta sesion que quieres plotear
...
% plot(RT.latency(ch)/1000,ones(1,length(ch))*1000,'*','markersize',13,'color',[0 0 .6])
plot(time(1,((RT.latency(ch(n_ripp))*srate/1000)-win):((RT.latency(ch(n_ripp))*srate/1000)+win)),ripples(n_ch,((RT.latency(ch(n_ripp))*srate/1000)-win):((RT.latency(ch(n_ripp))*srate/1000)+win))*2+1500,'color',[0 .4 0])
plot(RT.latency(ch(n_ripp))/1000,ones(1,1)*2000,'*','markersize',13,'color',[.6 0 0],'linewidth',2)
ylim([min(detrend(data(n_ch,((RT.latency(ch(n_ripp))*srate/1000)-win):((RT.latency(ch(n_ripp))*srate/1000)+win)))) max(ripples(n_ch,((RT.latency(ch(n_ripp))*srate/1000)-win):((RT.latency(ch(n_ripp))*srate/1000)+win))*2+1750)+250])
else
...
%% PLOT CON SUBPLOTS
% SELECCIONA EL CANAL (n_ch)
n_ch = 1; % elijo el canal a plotear
% n_ch = 4*(n_ch - 1) + 1; % como estan los cuatro canales de cada tetrodo, con esta linea se salta al tetrodo siguiente cuando n_ch = n_ch + 1;
...
% ELIGE EL RIPPLE QUE QUIERES PLOTEAR (n_ripp)
n_ripp =  2;% aqui seleccionas el ripple de esta sesion que quieres plotear
% figure(2)
win = .5*srate; % ventana de tiempo al rededor del ripple que se va a plotear
% plot(time,detrend(data(n_ch,:)),'k')
subplot(2,1,1)
...
print(gcf,'-dtiff',iname,'-r0')
disp(['figure ' num2str(example) ' (ripple ' num2str(n_ripp) ') saved'])
cd ..
cd ..
...
sessions = dir([base_dir rat_ripples(n_rat).name '\session*']);

% SELECCIONA LA SESION
n_ses = 1;
cd([base_dir rat_ripples(n_rat).name '\' sessions(n_ses).name])
...
plot([TTL(TTL(:,1)==11,6)/1000 TTL(TTL(:,1)==11,6)/1000],[-3000 6000],'g--','linewidth',2)
plot([TTL(TTL(:,1)~=11,6)/1000 TTL(TTL(:,1)~=11,6)/1000],[-3000 6000],'b--','linewidth',2)

title([sessions(n_ses).name ', TT' num2str(tetrodes(ceil(n_ch/4))) ', CH' num2str(channels(n_ch))],'interpreter','none')
set(gcf,'color','w')
%%
win = 6;
trial = 1;
