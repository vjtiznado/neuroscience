...
clear; clc;
tic
basal_path = 'E:\Gonzalo\Exp GV11 - GV12 Mayo 2018\'; % set the path where the folders of your different rats are located
...
r_dir = dir([basal_path 'GV12']); % get the folder's name of all your rats
for r = 1:size(r_dir,1)
    s_dir = dir([basal_path r_dir(r).name '\rec_02_OPR1']); % get the folder's name of all the recording days of this rat
    for s = 1:size(s_dir,1)
        disp(s_dir(s).name)
        wday_recs = dir([basal_path r_dir(r).name '\' s_dir(s).name '\rec_AM']);
        for ampm = 1:size(wday_recs,1)
            disp(wday_recs(ampm).name)
            cd([basal_path r_dir(r).name '\' s_dir(s).name '\' wday_recs(ampm).name])
            ttrs_dir_test = dir('*.ttr'); % detect all the '.ttr' files of one tetrode. If there is more than one .ttr file for one tetrode, it means that the code has to concatenate them
...
basal_dir = 'E:\Gonzalo\Exp GV11 - GV12 Mayo 2018\GV12\rec_03_OPR2\';
...
clear;clc;
basal_dir = 'E:\Gonzalo\Exp GV15 - GV16 Septiembre 2018\GV16\rec_05_OPR8\';
lan_dir = dir([basal_dir '\LAN*.mat']);
lan_for_plotting = 1;
