function [T,WV,n] = makeTTDAT2(chan,file_dat)
...
%         logind = false(size(TS,2)); % THIS MATRIX DOES NOT NEED TO BE SQUARED, IT CREATES OUT OF MEMORY ERRORS
        logind = false(1,size(TS,2)); % we just need a vector. NE & VT 02052019
        for c = chan
            logind( TS(2,:)==c ) = true;
        end
        T = TS(1, logind);
